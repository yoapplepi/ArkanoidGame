...
namespace ArkanoidGame
{
	...
	bool Platform::GetCollision(std::shared_ptr<Colladiable> collidable) const
	{
		...
		if (ballPosition.x < rect.left)
		{
			return sqr(ballPosition.x - rect.left) + sqr(ballPosition.y - rect.top)
				< sqr(BALL_SIZE / 2.0);
		}

		if (ballPosition.x > rect.left + rect.width) {
			return sqr(ballPosition.x - rect.left - rect.width) + sqr(ballPosition.y - rect.top) < sqr(BALL_SIZE / 2.0);
		}

		return std::fabs(ballPosition.y - rect.top) <= BALL_SIZE / 2.0;
	}
